...
// ==========================================
// 4. DELEGACIÓN DE EVENTOS GLOBAL REPARADA
// ==========================================
document.body.addEventListener('click', (e) => {
    const targetId = e.target.id || "";
    
    // CORRECCIÓN BOTÓN INTERFAZ: Detectar clicks en selección de modos
    if (targetId.includes('campaña') || targetId.includes('campana') || targetId.includes('infinito')) {
        // CORRECCIÓN REENTRADA: Si pulsas el botón del modo en el que YA estás jugando, solo te redirige a la pantalla del chat sin alterar la pregunta actual
        const modoDeseado = (targetId.includes('campaña') || targetId.includes('campana')) ? 'campaña' : 'infinito';
        
        if (gameState.modoActualJuego === modoDeseado && gameState.esperandoRespuesta) {
            switchView('view-chat'); // Te devuelve a la pantalla de juego limpia
            return;
        }

        // Si intentas cambiar de modo teniendo una ronda a medias, lo bloqueamos
        if (gameState.esperandoRespuesta) {
            console.log("Bloqueado: Termina la pregunta actual.");
            return; 
        }
    ...
    // Interceptar paneles generales de navegación (.btn-panel o IDs que salten vistas)
    ...
    if (btnPanel) {
        ...
        // CORRECCIÓN RETORNO: Asegurar mapeo correcto si el panel de juego se llama 'view-chat' en tu HTML
...
function cambiarModoEstrategia(modo) {
    ...
    const chatBox = document.getElementById('chat-messages');
    if (chatBox) chatBox.innerHTML = ""; // CORRECCIÓN: Limpieza radical al cambiar de modo
...
function nextRound() {
    ...
    // Si ya hay una pregunta activa renderizada en pantalla, salimos para evitar duplicados
    if (gameState.esperandoRespuesta) return; 
    ...
    // CORRECCIÓN ACUMULACIÓN: Vaciamos el chat al iniciar una ronda limpia para que solo se vea la conversación en curso
    const chatBox = document.getElementById('chat-messages');
    if (chatBox) chatBox.innerHTML = "";
