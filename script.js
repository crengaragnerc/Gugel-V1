...
function actualizarBotonesModoUI() {
    // Buscamos de forma flexible cualquier variante de botón de modo
...
document.body.addEventListener('click', (e) => {
    // Detectar si el clic se hizo en un botón de modo o DENTRO de él (el texto)
    ...
    // CORRECCIÓN RADICAL MODO INVITADO: Detección por ID o por texto literal ("preguntas infinitas")
    if (targetId.includes('campaña') || targetId.includes('campana') || targetId.includes('infinito') || targetText.includes('infinit') || targetText.includes('campa')) {
        ...
        // Si pulsamos el modo activo y estamos jugando, simplemente volvemos al chat visible sin romper nada
        if (gameState.modoActualJuego === modoDeseado && gameState.esperandoRespuesta) {
            switchView('view-chat');
            return;
        }

        // Si intentas saltar de modo con una ronda a medias, bloqueo absoluto
        if (gameState.esperandoRespuesta) {
            console.log("Acción bloqueada: Termina la pregunta actual.");
            return; 
        }
    ...
    // Interceptar paneles generales de navegación
...
function nextRound() {
    ...
    // Si ya estamos en medio de un ciclo de pregunta, evitamos duplicar la interfaz
    if (gameState.esperandoRespuesta) return; 
...
document.getElementById('chat-form').onsubmit = (e) => {
    ...
    // BLINDAJE CRÍTICO: Si el input está desactivado, el texto vacío, o YA se mostró el botón Continuar, ignoramos por completo
    if (!userText || !gameState.esperandoRespuesta || input.disabled || (continueBtn && continueBtn.style.display === "block")) return;
    
    // Desactivamos inmediatamente el input para congelar el envío múltiple
    input.disabled = true;
