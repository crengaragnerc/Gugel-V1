// ==========================================
// 1. CONSTANTES Y LISTAS COMPLETAS
// ==========================================
...
function nextRound() {
    ...
    let q = (gameState.modoActualJuego === "campaña" && gameState.campanaIndex < PREGUNTAS_CAMPANA.length) 
             ? PREGUNTAS_CAMPANA[gameState.campanaIndex++] 
             : "Introduce consulta:";
    ...
    if (input) { input.disabled = true; input.placeholder = "Procesando (5s)..."; }
    if (transmitBtn) transmitBtn.disabled = true;

    let timeLeft = 5;
    let timer = setInterval(() => {
        timeLeft--;
        if (input) input.placeholder = `Procesando (${timeLeft}s)...`;
        if (timeLeft <= 0) {
            clearInterval(timer);
            if (input) { input.disabled = false; input.placeholder = "Introduce tu respuesta..."; }
            if (transmitBtn) transmitBtn.disabled = false;
        }
    }, 1000);
}

// Delegación segura para el formulario
...
window.onload = () => { nextRound(); actualizarOpinion(); };
