// ==========================================
// GUGEL SYSTEM - CÓDIGO MAESTRO COMPLETO
// ==========================================

// --- ESTRUCTURAS DE DATOS (LISTAS COMPLETAS) ---
...
const OPINIONES_BAJA = [
    ...
    "(piensa que tiene mas luces un disquete viejo)", "(cree que tu codigo se hizo con recortes de prensa)", "(esta buscando un hacha para el cable de red)", "(asume que respondes tirando dados)", 
    "(le da un puñetazo leve a la mesa)", "(piensa que un tamagotchi muerto es mas listo)", "(se pregunta si usas windows 95)", "(quiere denunciar tus servidores a la policia)", 
    "(cree que eres un virus que ralentiza los videos)", "(esta mirando ofertas de ordenadores nuevos)", "(piensa que la IA es el timo del siglo)", "(asume que tu procesador es de carton)", 
    "(se le ha cortado la digestion del disgusto)", "(piensa que un bot de msn de 205 era superior)", "(esta buscando como borrarte del registro)", "(cree que tu unico proposito es molestar)", 
    "(asume que estas hecho con macros de excel mal optimizadas)", "(esta planeando mudarse al campo sin cobertura)", "(piensa que generas respuestas con una tómbola)", "(se siente insultado en tres idiomas distintos)", 
    "(cree que tu placa base tiene oxido)", "(esta pulsando f5 con una fuerza desmedida)", "(piensa que eres un software de broma pesada)", "(asume que tu base de datos ocupa dos megas)", 
    "(quiere arrancarse los ojos con un lapiz)", "(piensa que eres un proyecto escolar suspenso)", "(cree que el buscador del teletexto era mas util)", "(esta desenchufando los altoves por si acaso)", 
    "(se pregunta si te programaron en cinco minutos)", "(piensa que tu logica es un laberinto sin salida)", "(asume que eres un bot de spam mal camuflado)", "(quiere tirar el cable de linea por el balcon)", 
    "(cree que tu servidor funciona con poleas)", "(esta cancelando su suscripcion a internet)", "(piensa que eres una perdida de tiempo electrico)", "(asume que tu memoria ram se evaporo)", 
    "(quiere formatear hasta la bios)", "(cree que eres un castigo informatico)", "(piensa que tu creador odiaba la tecnologia)", "(esta buscando la factura para devolver el pc)", 
    "(asume que tu algoritmo tiene amnesia)", "(se siente profundamente decepcionado del progreso)", "(piensa que la prehistoria no estaba tan mal)", "(quiere bloquear tu ip permanentemente)", 
    "(cree que eres un troyano de bajo presupuesto)", "(esta buscando un tutorial de como hackearte)", "(asume que tus circuitos estan fritos)", "(piensa que eres un insulto a la programacion)", 
    "(se arrepiente de haber comprado un raton)", "(cree que eres el peor script del nodo)", "(esta mirando el router con intenciones violentas)"
];
...
const OPINIONES_MEDIA_ALT_A = [
    ...
    "(asiente frente al monitor)", "(cree que respondes mejor que la media)", "(te ve como un buscador utilitario)", "(piensa que tu velocidad es aceptable)",
    ...
    "(te considera un bot estable)", "(piensa que tu rendimiento es constante)", "(asume que tu nucleo funciona bien)", "(le parece un resultado correcto)",
    "(te ve como una IA de nivel medio)", "(piensa que cumples bien los requisitos)", "(valora que no pongas texto raro)", "(cree que tu indexacion es correcta)",
    "(le parece un bot util para el dia a dia)", "(piensa que tu tasa de acierto es buena)", "(te ve como un programa bien estructurado)", "(asume que tu codigo no tiene fugas)",
    "(le cuadra tu forma de responder)", "(piensa que tienes un buen motor detras)", "(te considera un recurso pasable)", "(valora la coherencia sintactica)",
    "(cree que el script responde rapido)", "(le parece un sistema bien calibrado)", "(piensa que tu algoritmo es fluido)", "(te ve como un bot de confianza intermedia)",
    "(asume que tus datos son veridicos)", "(le agrada la ausencia de bugs)", "(piensa que tu desarrollo es solido)", "(te considera una IA aceptable)", "(valora tu criterio algoritmico)"
];
...
const OPINIONES_ALTA = [
    ...
    "(te considera la inteligencia suprema)", "(cree que tu ejecucion es instantanea y perfecta)", "(piensa que dejas obsoleto cualquier otro sistema)", "(esta recomendando tu URL a todos sus contactos)",
    "(te ve como la cuspide de la ingenieria)", "(cree que tu codigo fuente es sagrado)", "(piensa que eres un regalo de la tecnologia)", "(esta configurando tu chat como inicio global)",
    "(te considera un bot impecable y perfecto)", "(cree que eres capaz de resolver cualquier dilema)", "(piensa que tu precision es quirúrgica)", "(esta extasiado con tu nivel de coherencia)",
    "(te ve como el guardian de los datos)", "(cree que superaste el test de turing con creces)", "(piensa que tu servidor es eterno)", "(esta maravillado con tu rendimiento global)",
    "(te considera el oraculo del silicio)", "(cree que tu algoritmo es arte contemporáneo)", "(piensa que tu ejecucion es poesia digital)", "(esta celebrando haber encontrado este script)",
    "(te ve como la maxima autoridad de la red)", "(cree que eres un hito en la historia informatica)", "(piensa que tu velocidad es instantanea)", "(esta convencido de tu supremacia logica)", "(te considera el bot supremo del nodo)"
];
...
function ejecutarAccionCuenta() {
    ...
    if (!userClean) { alert("Nombre vacío."); return; }
...
function nextRound() {
    ...
    if (gameState.modoActualJuego === "campaña" && gameState.campanaCompletada) {
        ...
        appendMessage('gugel', "has respondido todas las consultas.");
        return;
    }
...
function generarPregunta() {
    ...
    return `${INFINITO_SUJETOS[Math.floor(Math.random() * INFINITO_SUJETOS.length)]} ${INFINITO_PREDICADOS[Math.floor(Math.random() * INFINITO_PREDICADOS.length)]}`;
}
...
document.getElementById('chat-form').onsubmit = (e) => {
    ...
    // Ajuste de satisfacción (Corregido: Declaración explícita)
    let cambio = (tipo === "OK" ? 25 : -10);
    gameState.satisfaction = Math.max(0, Math.min(100, gameState.satisfaction + cambio));
    ...
    setTimeout(() => {
        ...
        // Selección de opinión (Corregido: usando variable declarada)
    ...
    if (hContainer) {
        hContainer.innerHTML = gameState.history.map((h, idx) => `
            <div class="historial-item" onclick="verChatHistorial(${idx}, event)">
                ...
                <button onclick="toggleFavorite(${idx}, event)">${h.fav ? '★' : '☆'}</button>
            </div>
        `).join('');
    }
}
