...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedUser) {
        ...
        // Si no hay usuario, nos aseguramos de mostrar la pantalla de login
...
function loginUser(username) {
    ...
    // Forzar el cambio de pantallas sin depender de clases CSS externas
    ...
    // Asignar el envío del formulario del chat
    chatForm.onsubmit = handleUserResponse;
    
    // Esperar un pelín a que el navegador asimile el cambio de pantalla antes de la primera pregunta
    setTimeout(() => {
        nextRound();
    }, 600);
}
...
// --- FLUJO REPARADO CON TEMPORIZADOR INMUNE ---
function nextRound(forcedQuestion = null) {
    gameState.roundStep = 1;
    userInput.disabled = true;
    sendBtn.disabled = true;
    ...
    setTimeout(() => {
        ...
        // Bucle de la cuenta atrás de 5 segundos
        const countdown = setInterval(() => {
            ...
            if (timeLeft > 0) {
                ...
                // Desbloqueo total del sistema
                gameState.roundStep = 2;
                userInput.disabled = false;
                sendBtn.disabled = false;
                userInput.placeholder = "Escribe tu respuesta como una IA profesional...";
                
                // Intento seguro de focus
                try {
                    userInput.focus();
                } catch(e) { console.log("Focus diferido"); }
